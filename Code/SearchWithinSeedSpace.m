function [lethalSets, nonLethalSets, growthRates, LPSolved] = SearchWithinSeedSpace...
    (model, maxCardinality, targetList, cutOff, Mode, cplxModel, grRateWT)
% SearchWithinSeedSpace finds all synthetic lethal sets within the "Seed
% Space" using FBA. This function uses parallel computation. 
%
% USAGE:
%
%    [lethalSets, nonLethalSets, growthRates, LPSolved] = SearchWithinSeedSpace...
%    (model, maxCardinality, targetList, cutOff, Mode, cplxModel, grRateWT)
%
% INPUT:
...
%    grRateWT:        maximum growth rate of the wild-type strain.
%                     (Default: [~, grRateWT] = buildCplexModel(model)).
%
% OUTPUTS:
...
for Stu = StuStart : min(maxCardinality, length(targetList))
    ...
    formatSpec = 'Solving %.0f LPs... \n';
    fprintf(formatSpec, nTotalPairs)
    % deletion analysis:
